...
typedef NTSTATUS(NTAPI *_LdrRegisterDllNotification)(ULONG Flags, PLDR_DLL_NOTIFICATION_FUNCTION NotificationFunction, PVOID Context, PVOID *Cookie);
...
void *GetLibraryHandleFromAddress(const void *address)
{
    ...
    HMODULE h = nullptr;
    if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, (LPWSTR)address, &h) == FALSE)
        return nullptr;
...
void *GetLibraryHandleFromAddress(const void *address)
{
    ...
    return dlopen(info.dli_fname, RTLD_NOLOAD | RTLD_NOW);
}
...
class LibraryImpl
{
    ...
    {
        void operator()(void *h) { System::Library::CloseLibrary(h); }
    };
  ...
  public:
    ...
    inline void CloseLibrary() { _Handle.reset(); }

    inline void *GetVSymbol(std::string const &symbol_name) const { return System::Library::GetSymbol(_Handle.get(), symbol_name.c_str()); }

    inline std::string GetLibraryPath() const { return System::Library::GetLibraryPath(_Handle.get()); }

    inline void *GetLibraryNativeHandle() const { return _Handle.get(); }

    inline bool IsLoaded() const { return _Handle != nullptr; }
};
