...
typedef int32_t(HOSTFXR_CALLTYPE *hostfxr_initialize_for_runtime_config_fn)(
    const char_t *runtime_config_path, const struct hostfxr_initialize_parameters *parameters, hostfxr_handle *host_context_handle);

typedef int32_t(HOSTFXR_CALLTYPE *hostfxr_get_runtime_delegate_fn)(const hostfxr_handle host_context_handle, enum hostfxr_delegate_type type, void **delegate);
...
struct dotnet_version_t
{
    ...
    inline std::string to_string() const { return std::to_string(major) + "." + std::to_string(minor) + "." + std::to_string(revision); }

    inline bool is_empty() const { return major == 0 && minor == 0 && revision == 0; }

    inline bool operator==(dotnet_version_t const &other) const { return major == other.major && minor == other.major && revision == other.revision; }

    inline bool operator<(dotnet_version_t const &other) const { return major < other.major || minor < other.minor || revision < other.revision; }
};
...
static std::string FindDotNetRoot()
{
    ...
    struct
    {
        ...
        (void *)RegistryManipulator::Roots::local_machine,
        "SOFTWARE\\dotnet\\Setup\\InstalledVersions\\" DOTNET_ARCH,
        "InstallLocation",
        RegistryManipulator::Rights::read,
        (void *)RegistryManipulator::Roots::local_machine,
        "SOFTWARE\\dotnet\\Setup\\InstalledVersions\\" DOTNET_ARCH,
        "InstallLocation",
        RegistryManipulator::Rights::read | RegistryManipulator::Rights::wow64_32key,
        (void *)RegistryManipulator::Roots::local_machine,
        "SOFTWARE\\dotnet\\Setup\\InstalledVersions\\" DOTNET_ARCH "\\sharedhost",
        "Path",
        RegistryManipulator::Rights::read};
...
class DotNetCoreHostImpl
{
  ...
  public:
    ...
    GetLoadAssemblyFunction(std::string const &assembly_name, std::string const &assembly_path, std::string config_path, hostfxr_delegate_type load_type)
    {
        ...
        result = _HostfxrGetRuntimeDelegate(ctx, load_type, (void **)&loadAssemblyFunction);
    ...
    bool LoadDotNetCoreHost(std::string const &dotnet_root, std::string const &dotnet_version)
    {
        ...
        if (_HostfxrInitializeForRuntimeConfig == nullptr || _HostfxrGetRuntimeDelegate == nullptr || _HostfxrClose == nullptr)
        {
    ...
    DefaultDelegateType LoadAssemblyAndEntryPoint(
        std::string const &assembly_entry_point_class,
        std::string const &assembly_entry_point_function,
        std::string const &assembly_path,
        std::string const &assembly_runtimeconfig)
    {
        auto assembly_name = _GetAssemblyName(assembly_entry_point_class);
        auto load_assembly_function =
            GetLoadAssemblyFunction(assembly_name, assembly_path, assembly_runtimeconfig, hostfxr_delegate_type::hdt_load_assembly_and_get_function_pointer);
        if (load_assembly_function == nullptr)
            return nullptr;
    ...
    void *GetFunctionDelegate(std::string const &full_qualified_class_name, std::string const &static_public_function_name, std::string const &full_qualifed_delegate_type)
    {
        ...
        auto load_assembly_function =
            GetLoadAssemblyFunction(assembly_name, it->second.AssemblyPath, it->second.AssemblyConfig, hostfxr_delegate_type::hdt_load_assembly_and_get_function_pointer);
        if (load_assembly_function == nullptr)
            return nullptr;
...
DefaultDelegateType DotNetCoreHost::LoadAssemblyAndEntryPoint(
    std::string const &assembly_entry_point_class, std::string const &assembly_entry_point_function, std::string const &assembly_path, std::string const &assembly_runtimeconfig)
{
...
void *
DotNetCoreHost::GetFunctionDelegate(std::string const &full_qualified_class_name, std::string const &static_public_function_name, std::string const &full_qualifed_delegate_type)
{
